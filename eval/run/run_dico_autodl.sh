...
do
  ...
  do
    ...
    OUTPUT_DIR="eval/outputs/${MODEL_NAME}_dicoplus+mpconf_APM${ACCELERATION_PARALLEL_METHOD}_PWT${POSITIONAL_WEIGHTS_TYPE}_DVDonly_imw${INITIAL_MIN_WEIGHT}_ur${UR_FACTOR}_${N_LIMIT:+limit_$N_LIMIT}/${TASKS}/SL${SL}_BL${BLOCK_LENGTH}/N${EXPLORATION_N}E${MAX_EXPLORATION_STEPS}_gate${MOPUP_GATE_RATIO}_factor${ACCELERATION_FACTOR}_exptr${EXPLORATION_THRESHOLD}_acctr${ACCELERATION_THRESHOLD}_mptr${MOPUP_MARGIN_THRESHOLD}"
    rm -rf $OUTPUT_DIR
    mkdir -p $OUTPUT_DIR
